...
for(run_pars_index in runsets){ 
	...
	shared_params_df_summary <- shared_params_df_summary %>% 
			mutate(lineage=case_when(lineage=="BX"~"Johnson & Johnson/Janssen", TRUE~"Pfizer-BioNTech")) 
	...
	meanvalsindiv <- meanvalsindiv %>% 
		mutate(lineage=case_when(
			(special==1)~"Johnson & Johnson/Janssen",
			(special==0)~"Pfizer-BioNTech"))
...
fig_ct_trajectory_vaxman <- make_sample_trajectory_special(shared_params_df, global_pars, siglevel=0.95, specialfill="red", referencefill="blue")	
...
meanvalsindiv_combined_forplotting_vaxman <- meanvalsindiv_combined %>% 
	ungroup() %>% 
	mutate(xval=case_when(lineage=="Pfizer-BioNTech"~1, lineage=="Johnson & Johnson/Janssen"~2)) %>% 
	select(dp, wp, wr, infdur, lineage, xval, breakthrough) %>% 
	rename(id=infdur) %>% 
	pivot_longer(c("dp","wp","wr","id")) %>% 
	rename(statistic=name) %>% 
	mutate(lineage=factor(lineage, levels=c("Pfizer-BioNTech","Johnson & Johnson/Janssen")))

shared_params_df_summary_forplotting_vaxman <- shared_params_df_summary %>% 
	mutate(xval=case_when(lineage=="Pfizer-BioNTech"~1, lineage=="Johnson & Johnson/Janssen"~2)) %>% 
	mutate(lineage=factor(lineage, levels=c("Pfizer-BioNTech","Johnson & Johnson/Janssen")))
...
fig_whiskers_dp_vaxman <- ggplot() + 
	...
	geom_beeswarm(data=filter(meanvalsindiv_combined_forplotting_vaxman, statistic=="dp"),
		aes(x=xval, y=global_pars[["lod"]]-value, col=lineage, shape=breakthrough), alpha=0.4, size=3, cex=4) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="dp"), 
	...
	scale_x_continuous(breaks=1:2, minor_breaks=1:2, label=c("Pfizer-BioNTech","Johnson & Johnson/\nJanssen")) + 
...
fig_whiskers_wp_vaxman <- ggplot() + 
	...
	geom_beeswarm(data=filter(meanvalsindiv_combined_forplotting_vaxman, statistic=="wp"),
		aes(x=xval, y=value, col=lineage, shape=breakthrough), alpha=0.4, size=3, cex=4) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="wp"), 
...
fig_whiskers_wr_vaxman <- ggplot() + 
	# geom_jitter(data=filter(meanvalsindiv_combined_forplotting, statistic=="wr"),
	# 	aes(x=xval, y=value, col=lineage), width=jitterwidth, height=0, alpha=0.4, size=3) + 
	geom_beeswarm(data=filter(meanvalsindiv_combined_forplotting_vaxman, statistic=="wr"),
		aes(x=xval, y=value, col=lineage, shape=breakthrough), alpha=0.4, size=3, cex=4) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="wr"), 
		aes(x=xval-whiskerwidth,xend=xval+whiskerwidth,y=mean,yend=mean),col="gray20", size=.75) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="wr"), 
		aes(x=xval-whiskerwidth/2,xend=xval+whiskerwidth/2,y=lwr,yend=lwr),col="gray20", size=.75) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="wr"), 
...
fig_whiskers_infdur_vaxman <- ggplot() + 
	# geom_jitter(data=filter(meanvalsindiv_combined_forplotting, statistic=="id"),
	# 	aes(x=xval, y=value, col=lineage), width=jitterwidth, height=0, alpha=0.4, size=3) + 
	geom_beeswarm(data=filter(meanvalsindiv_combined_forplotting_vaxman, statistic=="id"),
		aes(x=xval, y=value, col=lineage, shape=breakthrough), alpha=0.4, size=3, cex=4) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="id"), 
		aes(x=xval-whiskerwidth,xend=xval+whiskerwidth,y=mean,yend=mean),col="gray20", size=.75) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="id"), 
		aes(x=xval-whiskerwidth/2,xend=xval+whiskerwidth/2,y=lwr,yend=lwr),col="gray20", size=.75) +
	geom_segment(data=filter(shared_params_df_summary_forplotting_vaxman, statistic=="id"), 
...
ggsave(fig_whiskers_dp_vaxman, file=paste0(savedir_parent,"whiskers_dp_vaxman.pdf"), width=6, height=4)
ggsave(fig_whiskers_wp_vaxman, file=paste0(savedir_parent,"whiskers_wp_vaxman.pdf"), width=6, height=4)
ggsave(fig_whiskers_wr_vaxman, file=paste0(savedir_parent,"whiskers_wr_vaxman.pdf"), width=6, height=4)
ggsave(fig_whiskers_infdur_vaxman, file=paste0(savedir_parent,"whiskers_infdur_vaxman.pdf"), width=6, height=4)
ggsave(fig_ct_trajectory_vaxman, file=paste0(savedir_parent,"ct_trajectory_vaxman.pdf"), width=8, height=5)


ggsave(fig_whiskers_dp_vaxman, file=paste0(savedir_parent,"whiskers_dp_vaxman.png"), width=6, height=4)
ggsave(fig_whiskers_wp_vaxman, file=paste0(savedir_parent,"whiskers_wp_vaxman.png"), width=6, height=4)
ggsave(fig_whiskers_wr_vaxman, file=paste0(savedir_parent,"whiskers_wr_vaxman.png"), width=6, height=4)
ggsave(fig_whiskers_infdur_vaxman, file=paste0(savedir_parent,"whiskers_infdur_vaxman.png"), width=6, height=4)
ggsave(fig_ct_trajectory_vaxman, file=paste0(savedir_parent,"ct_trajectory_vaxman.png"), width=8, height=5)